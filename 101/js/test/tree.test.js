...
const SortedBinaryTree = require('../tree');
...
describe('SortedBinaryTree', () => {
  ...
  it('should tell if the tree contains a value', () => {
    const tree = buildTree();
    expect(tree.contains(55)).to.be.true;
  ...
  it('should remove a value', () => {
    const tree = buildTree();
    tree.remove(55);
    expect(tree.contains(55)).to.be.false;
  });
  ...
  it('should traverse the tree', () => {
    ...
    expect(dfsElements.length).to.be.equal(expectedSize);
    expect(bfsElements.length).to.be.equal(expectedSize);

    expect(dfsElements.slice(0, 3)).to.be.deep.equal(Array(7, 9, 10));
    expect(bfsElements.slice(0, 3)).to.be.deep.equal(Array(84, 44, 94));
  });
